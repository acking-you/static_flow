...
pub(crate) fn system_message_from_role_message(
...
pub(crate) fn cleaned_system_message_text(message: &SystemMessage) -> Option<String> {
    let content = strip_volatile_claude_code_billing_header(message.text.clone());
    (!content.trim().is_empty()).then_some(content)
}
pub(crate) fn build_injected_system_content(
    req: &MessagesRequest,
