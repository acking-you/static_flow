...
pub(crate) fn get_image_format_from_source(source: &super::types::ImageSource) -> Option<String> {
    detect_image_format_from_base64(&source.data)
        .or_else(|| get_image_format(&source.media_type))
        .map(str::to_string)
}
pub(crate) fn detect_image_format_from_base64(data: &str) -> Option<&'static str> {
    let mut prefix = data
        .chars()
...
pub(crate) fn detect_image_format_from_bytes(bytes: &[u8]) -> Option<&'static str> {
...
pub(crate) fn get_image_format(media_type: &str) -> Option<&'static str> {
    match media_type {
