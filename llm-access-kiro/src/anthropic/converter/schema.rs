...
pub(crate) fn permissive_object_schema() -> serde_json::Value {
    serde_json::json!({
        "type": "object",
        "properties": {},
        "required": [],
...
pub(crate) fn collect_schema_keywords(
...
pub(crate) fn schema_contains_multimodal_unsupported_keywords(value: &serde_json::Value) -> bool {
    match value {
...
pub(crate) fn content_value_contains_image(content: &serde_json::Value) -> bool {
...
pub(crate) fn request_message_contains_image(message: &super::types::Message) -> bool {
    content_value_contains_image(&message.content)
}
pub(crate) fn apply_multimodal_tool_schema_compatibility(tools: &mut [Tool], has_images: bool) {
    if !has_images {
        return;
    }
    for tool in tools {
