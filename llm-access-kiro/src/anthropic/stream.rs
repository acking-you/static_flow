...
//! ## Module map
//!
//! `stream.rs` is the facade: it owns the streaming state machine
//! (`SseStateManager`, `StreamContext`, `BufferedStreamContext`, `SseEvent`,
//! and the private `BlockState`/`ToolUseAccumulator` accumulators) together
//! with all their `impl` blocks, plus the shared constants. The pure helper
//! functions are grouped into descendant submodules:
//!
//! ```text
//!  Kiro upstream binary events
//!        |
//!        v
//!  StreamContext / BufferedStreamContext  (state machine, in parent)
//!        |
//!        +--> [usage]      input-token resolution + Anthropic usage JSON
//!        +--> [signature]  deterministic thinking-signature proto encoding
//!        +--> [thinking]   <thinking> tag detection/stripping in raw content
//!        +--> [text]       token estimation + structured-output JSON canon.
//!        |
//!        v
//!  Anthropic-compatible SSE events
//! ```
//!
//! Impl blocks stay in the parent so they keep private access to the state
//! machine's fields; submodule helpers are descendants and retain that access.
