...
pub fn spotlight() -> Html {
...

    {
        ...
        use_effect_with((), move |_| {
            ...
            let mouse_closure_opt: Option<Closure<dyn FnMut(MouseEvent)>> = window_opt.as_ref().map(|window| {
                let start_x = window
                    .inner_width()
                    .ok()
                    .and_then(|v| v.as_f64())
                ...
                let animation = Closure::wrap(Box::new(move || {
                    ...
                    if let Some(el) = spotlight_for_animation.cast::<HtmlElement>() {
                        let _ = el.style().set_property("left", &format!("{:.2}px", current.0));
                        let _ = el.style().set_property("top", &format!("{:.2}px", current.1));
                    }

                    if let Some(cb) = animation_cb_clone.borrow().as_ref() {
                        if let Ok(id) = window_for_animation.request_animation_frame(cb.as_ref().unchecked_ref()) {
            ...
            move || {
                if let (Some(window), Some(mouse_closure)) = (window_opt.as_ref(), mouse_closure_opt.as_ref()) {
