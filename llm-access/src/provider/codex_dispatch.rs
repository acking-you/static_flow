//! Codex upstream proxy dispatch and response adaptation/streaming, plus Codex
//! account cooldown classification.

#[allow(unused_imports, reason = "submodule inherits parent facade imports via glob")]
use super::*;

pub(crate) async fn dispatch_codex_proxy(
...
pub(crate) async fn adapt_codex_upstream_response(
...
pub(crate) async fn adapt_codex_upstream_response_from_parts(
...
pub(crate) fn codex_quota_exhaustion_cooldown(
    status: StatusCode,
    bytes: &Bytes,
) -> Option<Duration> {
    if !matches!(
...
pub(crate) fn codex_message_indicates_usage_limit(message: &str) -> bool {
...
pub(crate) fn randomized_codex_transient_account_failure_cooldown<R: Rng + ?Sized>(
    rng: &mut R,
) -> Duration {
...
pub(crate) fn codex_temporary_request_failure_cooldown(
    status: StatusCode,
    bytes: &Bytes,
) -> Option<Duration> {
...
pub(crate) fn mark_codex_transient_request_failure_cooldown(
...
pub(crate) fn codex_status_from_error_json_value(value: &Value) -> Option<StatusCode> {
...
pub(crate) fn stream_codex_upstream_response(
    response: reqwest::Response,
    status: StatusCode,
