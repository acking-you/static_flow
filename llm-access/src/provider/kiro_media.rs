//! Kiro remote-media resolution: media-type canonicalization/detection,
//! private-IP rejection, and remote image/document source building.

#[allow(unused_imports, reason = "submodule inherits parent facade imports via glob")]
use super::*;

pub(crate) fn kiro_remote_media_accept_header(kind: KiroRemoteMediaKind) -> &'static str {
    match kind {
...
pub(crate) async fn resolve_kiro_remote_media_sources(
...
pub(crate) fn payload_has_kiro_remote_media_sources(payload: &MessagesRequest) -> bool {
...
pub(crate) fn strip_kiro_remote_media_sources(
    payload: &mut MessagesRequest,
...
pub(crate) fn stripped_kiro_remote_media_source(
...
pub(crate) fn summarize_kiro_remote_media_url(raw_url: &str) -> String {
...
pub(crate) fn is_kiro_remote_media_source_block(item: &serde_json::Value) -> bool {
...
pub(crate) async fn resolve_kiro_remote_media_sources_with_fetcher(
    payload: &mut MessagesRequest,
...
pub(crate) fn pending_kiro_remote_media_source(
    item: &serde_json::Value,
    message_index: usize,
    block_index: usize,
...
pub(crate) fn build_kiro_remote_image_source(
...
pub(crate) fn build_kiro_remote_document_source(
    source_media_type: Option<&str>,
    response_media_type: Option<&str>,
    url: &str,
    bytes: &[u8],
...
pub(crate) fn normalize_media_type(value: &str) -> Option<String> {
    value
        .split(';')
        .next()
        .map(str::trim)
...
pub(crate) fn canonical_image_media_type(media_type: &str) -> Option<&'static str> {
    match media_type {
...
pub(crate) fn canonical_document_media_type(media_type: &str) -> Option<&'static str> {
    match media_type {
...
pub(crate) fn image_media_type_from_url(url: &str) -> Option<&'static str> {
...
pub(crate) fn document_media_type_from_url(url: &str) -> Option<&'static str> {
    match lower_url_path_extension(url).as_deref() {
...
pub(crate) fn lower_url_path_extension(url: &str) -> Option<String> {
...
pub(crate) fn validate_kiro_remote_media_url(
    raw_url: &str,
...
pub(crate) async fn validate_kiro_remote_media_resolved_addresses(
    url: &url::Url,
) -> Result<(), KiroRemoteMediaResolutionError> {
    let host = url
        .host_str()
...
pub(crate) fn reject_private_kiro_remote_media_ip(
    ip: IpAddr,
) -> Result<(), KiroRemoteMediaResolutionError> {
...
pub(crate) fn is_private_kiro_remote_media_ip(ip: IpAddr) -> bool {
    match ip {
        IpAddr::V4(ip) => is_private_kiro_remote_media_ipv4(ip),
        IpAddr::V6(ip) => is_private_kiro_remote_media_ipv6(ip),
    }
}
pub(crate) fn is_private_kiro_remote_media_ipv4(ip: Ipv4Addr) -> bool {
    ip.is_private()
        || ip.is_loopback()
        || ip.is_link_local()
        || ip.is_broadcast()
...
pub(crate) fn is_private_kiro_remote_media_ipv6(ip: Ipv6Addr) -> bool {
    ip.is_loopback()
        || ip.is_unique_local()
