//! Kiro usage summary/JSON assembly, cached-token estimation, billable-token
//! computation, and usage/preflight recording.

#[allow(unused_imports, reason = "submodule inherits parent facade imports via glob")]
use super::*;

pub(crate) fn build_kiro_usage_summary(
    model: &str,
...
pub(crate) fn anthropic_usage_json_with_policy(
...
pub(crate) fn anthropic_usage_json_from_summary_with_policy(
...
pub(crate) fn estimate_formula_cached_tokens(
    model: &str,
    input_tokens_total: i32,
    output_tokens: i32,
...
pub(crate) fn estimate_prefix_cached_tokens(
...
pub(crate) fn zero_kiro_usage_summary() -> KiroUsageSummary {
    KiroUsageSummary {
...
pub(crate) async fn record_kiro_preflight_failure(record: KiroPreflightFailureRecord<'_>) {
...
pub(crate) async fn record_kiro_usage(record: KiroUsageRecord<'_>) -> anyhow::Result<()> {
...
pub(crate) async fn record_kiro_websearch_usage(
...
pub(crate) fn kiro_billable_tokens(
    model: &str,
    usage: KiroUsageSummary,
    cache_ctx: &KiroCacheContext,
) -> u64 {
...
pub(crate) fn kiro_billable_tokens_with_multipliers(
    model: &str,
    usage: KiroUsageSummary,
