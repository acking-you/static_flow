//! Route selection and hydration with account permits, ordered Kiro route
//! selection, and Kiro failover decisions.

#[allow(unused_imports, reason = "submodule inherits parent facade imports via glob")]
use super::*;

pub(crate) async fn select_codex_route_with_account_permit(
...
pub(crate) async fn select_kiro_route_with_account_permit(
...
pub(crate) async fn hydrate_codex_route_for_dispatch(
...
pub(crate) fn selection_ordered_kiro_routes<'a>(
...
pub(crate) fn has_remaining_kiro_candidate(
    routes: &[ProviderKiroRoute],
    failed_accounts: &HashSet<String>,
...
pub(crate) async fn should_failover_after_kiro_route_failure(
...
pub(crate) fn account_names_for_kiro_routing_identity(
    routes: &[ProviderKiroRoute],
...
pub(crate) fn proxy_cooldown_key_for_route(route: &ProviderKiroRoute) -> Option<String> {
    route
        .proxy
        .as_ref()
        .map(|proxy| format!("url:{}", proxy.proxy_url))
}
pub(crate) fn is_kiro_data_plane_route(path: &str) -> bool {
    provider_route_requirement(path)
        .map(|requirement| requirement.provider_type == ProviderType::Kiro)
        .unwrap_or(false)
}
pub(crate) fn is_codex_anthropic_messages_route(path: &str) -> bool {
    normalized_codex_gateway_path(path) == Some("/v1/messages")
}
pub(crate) fn key_matches_route(key: &AuthenticatedKey, path: &str) -> bool {
    if path == "/v1/models" {
        return true;
    }
