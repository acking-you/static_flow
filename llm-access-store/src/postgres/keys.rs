//! Admin API key storage: key-bundle load/list/page, summaries, candidate
//! credit summaries, account-group references, upsert/disable, plus the
//! `AdminKeyStore` impl.
...
impl PostgresControlRepository {
    pub(crate) async fn load_key_hashes_by_ids(
        &self,
        key_ids: &[String],
    ...
    pub(crate) async fn load_key_bundle_by_id(
        &self,
        key_id: &str,
    ...
    pub(crate) async fn list_key_bundles(&self) -> anyhow::Result<Vec<KeyBundle>> {
        self.ensure_connection_alive()?;
        let rows = self
            .client
            .query(
    ...
    pub(crate) async fn admin_keys_summary(
        &self,
        provider_type: Option<&str>,
    ...
    pub(crate) async fn list_key_bundles_page(
        &self,
        provider_type: Option<&str>,
        page: AdminPageRequest,
    ) -> anyhow::Result<AdminKeysPage> {
    ...
    pub(crate) async fn list_kiro_key_bundles_page_with_candidate_summaries(
        &self,
        page: AdminPageRequest,
    ) -> anyhow::Result<AdminKeysPage> {
        self.ensure_connection_alive()?;
    ...
    pub(crate) async fn find_key_referencing_account_group(
        &self,
        provider_type: &str,
        group_id: &str,
    ) -> anyhow::Result<Option<AdminKey>> {
    ...
    pub(crate) async fn upsert_key_bundle_client(
        client: &SqlxClient,
        key: &KeyRecord,
    ...
    pub(crate) async fn disable_admin_key_if_present(
        &self,
        key_id: &str,
        updated_at_ms: i64,
    ) -> anyhow::Result<()> {
