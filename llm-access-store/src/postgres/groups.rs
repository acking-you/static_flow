//! Admin account-group storage: group row load/list/page plus the
//! `AdminAccountGroupStore` impl.
...
impl PostgresControlRepository {
    pub(crate) async fn list_admin_account_groups_rows(
        &self,
        provider_type: &str,
    ) -> anyhow::Result<Vec<AdminAccountGroup>> {
        self.ensure_connection_alive()?;
    ...
    pub(crate) async fn list_admin_account_groups_page_rows(
        &self,
        provider_type: &str,
        page: AdminPageRequest,
    ) -> anyhow::Result<core_store::AdminAccountGroupsPage> {
    ...
    pub(crate) async fn get_admin_account_group_row(
        &self,
        group_id: &str,
    ) -> anyhow::Result<Option<AdminAccountGroup>> {
        self.ensure_connection_alive()?;
