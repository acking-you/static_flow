...
//! keeps the shared status/model constants, two generic value helpers, and the
//! unit tests, while delegating each concern to a focused submodule:
//!
//! ```text
//!  incoming bytes
//!        |
//!        v
//!  [prepare]            read body, decode, drive normalization (entry points)
//!        |
//!        +--> [policy]            gpt-5.3-codex spark / fast tier / billing
//!        +--> [chat_completions]  OpenAI chat-completions -> /responses shape
//!        +--> [tools]             tool-schema + OpenAI tool-name (de)mangling
//!        +--> [native_responses]  native /responses repair + validation
//!        +--> [normalization]     upstream base-url, model, field filtering
//!        +--> [last_message]      extract last-message preview (logging)
//!        +--> [headers]           header / client-IP / origin / query parsing
