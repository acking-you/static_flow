...
pub struct KiroEventContext {
    ...
    /// Time spent before a serving account was selected and locally admitted.
    pub routing_wait_ms: Option<i32>,
    /// Time from final upstream request send until upstream response headers.
    pub upstream_headers_ms: Option<i32>,
    /// Time from upstream response headers until the response body was fully
    /// consumed and transformed into a usage event.
    pub post_headers_body_ms: Option<i32>,
    /// Downstream request body size read by the handler.
    pub request_body_bytes: Option<u64>,
    /// Time spent reading the downstream request body.
    pub request_body_read_ms: Option<i32>,
    /// Time spent parsing the downstream JSON body.
    pub request_json_parse_ms: Option<i32>,
    /// Time between request middleware entry and handler-owned work.
    pub pre_handler_ms: Option<i32>,
    /// Time until the first SSE bytes are yielded to the downstream client.
    pub first_sse_write_ms: Option<i32>,
    /// Time until the gateway finishes consuming/transforming the upstream
    /// response stream.
    pub stream_finish_ms: Option<i32>,
    /// Number of monthly-quota failovers before the selected account succeeded.
    pub quota_failover_count: u64,
    /// JSON diagnostics for account selection, local throttling, cooldowns,
    /// and account-level failover decisions.
    pub routing_diagnostics_json: Option<String>,
    /// Monotonic timestamp recorded when upstream response headers arrived.
...
fn build_kiro_usage_event_record(
    ...
    let post_headers_body_ms = build.event_context.post_headers_body_ms.or_else(|| {
        build
            .event_context
            .upstream_headers_at
            .map(|started| started.elapsed().as_millis().min(i32::MAX as u128) as i32)
    });
    LlmGatewayUsageEventRecord {
        ...
        latency_ms,
        routing_wait_ms: build.event_context.routing_wait_ms,
        upstream_headers_ms: build.event_context.upstream_headers_ms,
        post_headers_body_ms,
        request_body_bytes: build.event_context.request_body_bytes,
        request_body_read_ms: build.event_context.request_body_read_ms,
        request_json_parse_ms: build.event_context.request_json_parse_ms,
        pre_handler_ms: build.event_context.pre_handler_ms,
        first_sse_write_ms: build.event_context.first_sse_write_ms,
        stream_finish_ms: build.event_context.stream_finish_ms.or(Some(latency_ms)),
        quota_failover_count: build.event_context.quota_failover_count,
        routing_diagnostics_json: build.event_context.routing_diagnostics_json.clone(),
